...
def tensor_to_image(x):   
    ...
    #ndarr = x.squeeze(dim=0).mul_(255).round().clamp_(0, 255).permute(1, 2, 0).squeeze().to('cpu', torch.uint8).numpy()
    ndarr = x.squeeze(dim=0).round().clamp_(0, 255).permute(1, 2, 0).squeeze().to('cpu', torch.uint8).numpy()
