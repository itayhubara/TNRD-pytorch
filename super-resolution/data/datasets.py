...
class DatasetSR(torch.utils.data.dataset.Dataset):
    ...
    def __getitem__(self, index):
        ...
        input = color.rgb2ycbcr(input).astype(np.float32)
        target = color.rgb2ycbcr(target).astype(np.float32)
        input = self.image_transform(input)[0].unsqueeze(0)
        target = self.image_transform(target)[0].unsqueeze(0)
        #print(input.shape,input.max(),target.max())
        #print(target.shape)
        
        #if not self.training:
        #    print('loader',target.max(),target.min())
